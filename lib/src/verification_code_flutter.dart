...
class VerificationCode extends StatefulWidget {
  ...
  const VerificationCode({
    ...
    this.length = 4,
    this.cursorColor,
    this.itemSize = const Size(50, 50),
    this.underlineColor,
...
class _VerificationCodeState extends State<VerificationCode> {
  ...
  Widget _buildInputItem(int index) {
    final padding =
        widget.padding ?? EdgeInsets.all(((widget.itemSize.width * 2) / 10));
    ...
    return KeyboardListener(
      ...
      child: Center(
        child: TextField(
          ...
          showCursor: (widget.showCursor ?? true) && !_isCompleted,
          ...
          decoration: widget.full ? fullDecoration : underlinedDecoration,
  ...
  List<Widget> _buildListWidget() {
    ...
    for (int index = 0; index < widget.length; index++) {
      double left = (index == 0) ? 0.0 : (widget.itemSize.width / 10);
